...
devenv() { 
	...
	if [ -n "$AGENT" ]; then
		docker run --rm -ti -e DEVENV=$TAG -e DEVENV_VOLUME=$DEVENV_VOLUME -v$DEVENV_VOLUME/state:/state -v $DEVENV_VOLUME:$DEVENV_VOLUME -v /var/run/docker.sock:/var/run/docker.sock -v ${LOCAL_STATE}/$AGENT:/tmp/ssh-agent.sock --env SSH_AUTH_SOCK=/tmp/ssh-agent.sock --name $CONTAINER_NAME $IMAGE_NAME; 
	else
